        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <div class="flex items-center mt-3 space-x-4 text-sm">
                    <span class="theme-aware-bg-card/20 px-2 py-1 rounded">{{ $tenant->domain }}</span>
                    <span class="theme-aware-bg-card/20 px-2 py-1 rounded">{{ ucfirst($tenant->status) }}</span>
                    <span class="theme-aware-bg-card/20 px-2 py-1 rounded">{{ $tenant->users_count ?? 0 }} Users</span>
                </div>
            </div>
    ...
    <form action="{{ route('admin.tenants.update', $tenant) }}" method="POST" id="editTenantForm" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            @error('max_users')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                            <p class="mt-1 text-xs theme-aware-text-muted">Current users: {{ $tenant->users_count ?? 0 }}</p>
                        </div>
                        ...
                        <div>
                            ...
                            <select id="status" 
                                    name="status" 
                                    class="w-full px-4 py-3 border theme-aware-border rounded-lg focus:ring-2 focus:ring-primary focus:theme-aware-border-focus @error('status') border-red-500 @enderror">
                        ...
                        <div>
                            <label for="trial_ends_at" class="block text-sm font-medium theme-aware-text-secondary mb-2">
                                Trial End Date
                            </label>
                            <input type="date" 
                                   id="trial_ends_at" 
                                   ...
                                   class="w-full px-4 py-3 border theme-aware-border rounded-lg focus:ring-2 focus:ring-primary focus:theme-aware-border-focus @error('trial_ends_at') border-red-500 @enderror">
                            @error('trial_ends_at')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                ...
                <div class="theme-aware-bg-card rounded-xl shadow-lg p-6">
                    ...
                    <div class="space-y-6">
                        ...
                        {{-- Feature Toggles --}}
                        <div>
                            <h3 class="text-lg font-semibold theme-aware-text mb-4">Feature Access</h3>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <label class="flex items-center">
                                    ...
                                    <span class="ml-2 text-sm theme-aware-text-secondary">API Access</span>
                                </label>
                                ...
                                <label class="flex items-center">
                                    <input type="checkbox" 
                                           ...
                                           class="rounded theme-aware-border text-blue-600 focus:ring-primary">
                                    <span class="ml-2 text-sm theme-aware-text-secondary">Advanced Reporting</span>
                                </label>
                                ...
                                <label class="flex items-center">
                                    <input type="checkbox" 
                                           ...
                                           class="rounded theme-aware-border text-blue-600 focus:ring-primary">
                                    <span class="ml-2 text-sm theme-aware-text-secondary">White Label</span>
                                </label>
                                ...
                                <label class="flex items-center">
                                    <input type="checkbox" 
                                           ...
                                           class="rounded theme-aware-border text-blue-600 focus:ring-primary">
                                    <span class="ml-2 text-sm theme-aware-text-secondary">Custom Integrations</span>
                                </label>
                            </div>
                        </div>
                    </div>
            ...
            <div class="space-y-6">
                {{-- Tenant Statistics --}}
                <div class="theme-aware-bg-card rounded-xl shadow-lg p-6">
                    <h3 class="text-lg font-bold theme-aware-text mb-4">Tenant Statistics</h3>
                    
                    <div class="space-y-4">
                        <div class="flex justify-between items-center">
                            <span class="theme-aware-text-secondary">Total Users:</span>
                            <span class="font-semibold">{{ $tenant->users_count ?? 0 }}</span>
                        </div>
                        
                        <div class="flex justify-between items-center">
                            <span class="theme-aware-text-secondary">Active Projects:</span>
                            <span class="font-semibold">{{ $tenant->projects_count ?? 0 }}</span>
                        </div>
                        
                        <div class="flex justify-between items-center">
                            <span class="theme-aware-text-secondary">Created:</span>
                            <span class="font-semibold text-sm">{{ $tenant->created_at->format('M j, Y') }}</span>
                        </div>
                        
                        <div class="flex justify-between items-center">
                            <span class="theme-aware-text-secondary">Last Updated:</span>
                ...
                {{-- Recent Activity --}}
                <div class="theme-aware-bg-card rounded-xl shadow-lg p-6">
                    <h3 class="text-lg font-bold theme-aware-text mb-4">Recent Activity</h3>
                        ...
                        @if(isset($tenant->recent_activity) && $tenant->recent_activity->count() > 0)
                            @foreach($tenant->recent_activity->take(5) as $activity)
                                <div class="flex items-start space-x-2">
                                    ...
                                    <div>
                                        <p class="theme-aware-text">{{ $activity->description }}</p>
                                        <p class="theme-aware-text-muted text-xs">{{ $activity->created_at->diffForHumans() }}</p>
                                    </div>
                                </div>
                            @endforeach
                        @else
                            <p class="theme-aware-text-muted text-center py-4">No recent activity</p>
                        @endif
                    </div>
                </div>
                ...
                <div class="theme-aware-bg-card rounded-xl shadow-lg p-6">
                    <div class="space-y-4">
                        ...
                        <a href="{{ route('admin.tenants.show', $tenant) }}" 
                           class="w-full theme-aware-bg-tertiary theme-aware-text-secondary py-3 px-6 rounded-lg font-semibold hover:bg-gray-300 transition text-center block">
                            <i class="fas fa-eye mr-2"></i>
                            View Details
                        </a>
                        
                        <a href="{{ route('admin.tenants.index') }}" 
                           class="w-full theme-aware-bg-secondary theme-aware-text-secondary py-3 px-6 rounded-lg font-semibold hover:theme-aware-bg-tertiary transition text-center block">
