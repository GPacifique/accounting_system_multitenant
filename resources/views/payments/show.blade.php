        ...
        <div class="theme-aware-bg-card rounded-xl shadow-lg overflow-hidden">
            <div class="p-6 sm:p-8">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <h3 class="text-lg font-semibold text-gray-700 border-b pb-2 mb-4">Payment Info</h3>
                        <dl class="space-y-4">
                            <div class="flex flex-col">
                                <dt class="text-sm font-medium theme-aware-text-muted">Payer</dt>
                                <dd class="mt-1 text-md text-gray-900">{{ $payment->payer_name ?? ($payment->user->email ?? '—') }}</dd>
                            </div>
                            <div class="flex flex-col">
                                <dt class="text-sm font-medium theme-aware-text-muted">Method</dt>
                                <dd class="mt-1 text-md text-gray-900">{{ $payment->payment_method ?? '—' }}</dd>
                            </div>
                            <div class="flex flex-col">
                                <dt class="text-sm font-medium theme-aware-text-muted">Reference</dt>
                                <dd class="mt-1 text-md text-gray-900">{{ $payment->reference ?? '—' }}</dd>
                            </div>
                        </dl>
                    </div>
                    <div>
                        <h3 class="text-lg font-semibold text-gray-700 border-b pb-2 mb-4">Additional</h3>
                        <dl class="space-y-4">
                            <div class="flex flex-col">
                                <dt class="text-sm font-medium theme-aware-text-muted">Created at</dt>
                                <dd class="mt-1 text-md text-gray-900">{{ optional($payment->created_at)->format('F j, Y g:i A') ?? '—' }}</dd>
                            </div>
